      ...
      <main className="mx-auto max-w-6xl px-6 py-14">
        {currentPage === "leaderboard" ? (
          <div className="space-y-6">
            ...
            {/* podium for the top 3 — silver, gold, bronze layout with 1st in the middle */}
            {leaderboardData.length >= 3 && (
              <div className="grid grid-cols-3 gap-4">
                {[
                  { entry: leaderboardData[1], rank: 2, label: "2nd", ring: "border-slate-400/40", badge: "bg-slate-400/10 text-slate-300", glow: "" },
                  { entry: leaderboardData[0], rank: 1, label: "1st", ring: "border-yellow-400/40", badge: "bg-yellow-400/10 text-yellow-300", glow: "shadow-[0_0_40px_rgba(250,204,21,0.12)]" },
                  { entry: leaderboardData[2], rank: 3, label: "3rd", ring: "border-orange-400/40", badge: "bg-orange-400/10 text-orange-300", glow: "" },
                ].map(({ entry, rank, label, ring, badge, glow }) => (
                  <div key={rank} className={`flex flex-col items-center rounded-3xl border ${ring} bg-white/5 p-6 text-center ${glow} ${rank === 1 ? "scale-105" : ""}`}>
                    <span className={`rounded-full px-3 py-1 text-xs font-bold ${badge}`}>{label}</span>
                    {/* showing the shortened wallet address — names would break anonymity */}
                    <p className="mt-3 font-mono text-sm font-bold text-white">{shortAddress(entry.wallet_address)}</p>
                    <p className="mt-1 text-3xl font-black text-white">{entry.balance}</p>
                    <p className="text-sm text-slate-400">BCU tokens</p>
                  </div>
                ))}
              </div>
            )}
